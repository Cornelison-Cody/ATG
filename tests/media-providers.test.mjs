...
test("image provider adapter decodes provider bytes and moderation blocks flagged prompts", async () => {
  ...
  try {
    ...
    const generated = await generateMediaWithProvider({ job, apiKey: "test-key" });
...
test("sound provider is configurable and unsupported media stays unavailable", async () => {
  ...
  globalThis.fetch = async () => new Response(new Uint8Array([1, 2]), { status: 200, headers: { "content-type": "audio/wav" } });
  try {
    const result = await generateMediaWithProvider({ job: { id: "job", kind: "sound-effect", provider: "openai-sfx", model: "sfx", prompt: "buzzer" } });
