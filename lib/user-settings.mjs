...
export async function saveUserApiKey(userId, apiKey) {
  ...
  await updateUserSettings(userId, (record) => {
    record.encryptedApiKey = encryptApiKey(normalized, userId);
  });
}
...
export async function deleteUserApiKey(userId) {
  await updateUserSettings(userId, (record) => {
    ...
    if (record.aiBillingMode === AI_BILLING_MODES.BYOK) {
      record.aiBillingMode = AI_BILLING_MODES.MANAGED;
    }
  });
}

export async function getUserAiBillingMode(userId) {
  const record = await readUserSettings(userId);
  return normalizeAiBillingMode(record?.aiBillingMode || AI_BILLING_MODES.MANAGED);
}

export async function saveUserAiBillingMode(userId, mode) {
  const aiBillingMode = normalizeAiBillingMode(mode);
  if (aiBillingMode === AI_BILLING_MODES.BYOK && !(await hasUserApiKey(userId))) {
    throw new UserSettingsError("Save and test a personal OpenAI API key before switching to BYOK.", 400);
  }
  await updateUserSettings(userId, (record) => {
    record.aiBillingMode = aiBillingMode;
  });
...
export function normalizeAiBillingMode(mode) {
  const normalized = typeof mode === "string" ? mode.trim().toLowerCase() : "";
  if (normalized === AI_BILLING_MODES.MANAGED || normalized === AI_BILLING_MODES.BYOK) {
    return normalized;
  }
  throw new UserSettingsError("AI billing mode must be managed or byok.", 400);
}
...
async function updateUserSettings(userId, mutate) {
  const id = userDocumentId(userId);
  if (useAzureStorage()) {
    ...
    try {
      ...
      record = resource || { id, type: "user-settings" };
    } catch (error) {
      if (error?.code !== 404) {
        throw error;
      }
      record = { id, type: "user-settings" };
    }
    mutate(record);
    record.id = id;
    record.type = "user-settings";
    record.updatedAt = new Date().toISOString();
    await container.items.upsert(record);
    return;
  }
  await updateLocalSettings((database) => {
    const record = database.records[id] || { id, type: "user-settings" };
    mutate(record);
    record.id = id;
    record.type = "user-settings";
    record.updatedAt = new Date().toISOString();
    database.records[id] = record;
  });
}
