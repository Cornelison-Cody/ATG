...
export function validateGameTextPath(filePath) {
  ...
  if (!gameTextExtensions.has(extensionFor(normalizedPath))) {
    throw new GameFileValidationError("Game file type is not editable by the companion.");
  }
...
export function normalizeGameTextFiles(files) {
  ...
  const normalizedFiles = files.map((file) => {
    ...
    if (size > maxGameTextFileBytes) {
      throw new GameFileValidationError(`${filePath} exceeds the companion file size limit.`, 413);
    }

    totalBytes += size;
    if (totalBytes > maxGameTextTotalBytes) {
      throw new GameFileValidationError("Game files exceed the companion upload size limit.", 413);
    }
