...
export async function getAiBillingStatus(userId) {
  ...
  return {
    ...
    managed: {
      ...
      eligible: isManagedAiEligible(userId),
...
export function isManagedAiEligible(userId) {
  const allowlist = managedAiBetaAllowlist();
  if (allowlist.has("*")) {
    return true;
  }
  return allowlist.has(normalizeAllowlistEntry(userId));
}

export function managedAiBetaAllowlist() {
  return new Set(
