export async function generateMediaWithProvider({ job, apiKey, signal }) {
  if (job.provider === "openai-image") return openAiImage(job, apiKey, signal);
  if (job.provider === "openai-sfx") return configuredSoundEffect(job, signal);
  throw new Error("Unsupported media provider.");
}

export async function moderateMediaWithProvider({ job, generated, apiKey, signal }) {
  if (job.kind === "sound-effect" || !apiKey) return { allowed: true, label: "not_checked" };
...
async function openAiImage(job, apiKey, signal) {
  if (!apiKey) throw new Error("An OpenAI API key is required for image generation.");
  const response = await fetch("https://api.openai.com/v1/images/generations", { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ model: job.model, prompt: job.prompt, size: "1024x1024", response_format: "b64_json" }), signal });
...
async function configuredSoundEffect(job, signal) {
  const endpoint = process.env.ATG_SFX_PROVIDER_URL;
  if (!endpoint) throw new Error("Sound-effect provider is not configured.");
  const response = await fetch(endpoint, { method: "POST", headers: { "Content-Type": "application/json", ...(process.env.ATG_SFX_PROVIDER_KEY ? { Authorization: `Bearer ${process.env.ATG_SFX_PROVIDER_KEY}` } : {}) }, body: JSON.stringify({ model: job.model, prompt: job.prompt }), signal });
