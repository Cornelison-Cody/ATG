...
export function compressSoundEffect({ bytes, contentType = "audio/wav", sampleRate = 44100 } = {}) {
  ...
  const outputType = contentType === "audio/ogg" || contentType === "audio/mpeg" || contentType === "audio/mp4" ? contentType : "audio/ogg";
  return { bytes, contentType: outputType, sampleRate, codec: outputType === "audio/ogg" ? "opus" : "aac", loop: false };
}
