...
import { exportGameTextFiles, listUploadedGameAssets, readGameAsset, readGameConfig, replaceGameTextFilesAtomically } from "./project-game";
import { normalizeConvertedGameFiles, validateConvertedGame } from "./conversion-validation.mjs";
import {
  acceptConversion,
  cancelConversion,
...
export async function completeConversion(conversionId, changedFiles, finalMessage, warnings = []) {
  ...
  const textFiles = normalizeConvertedGameFiles(
    [...source.entries()].map(([path, content]) => ({ path, content }))
  ...
  return saveConversionCandidate(conversionId, {
    ...
    candidateRevision: createHash("sha256").update(JSON.stringify(textFiles)).digest("hex").slice(0, 16),
...
export async function acceptConversionCandidate(project, conversionId, acknowledgeWarnings = false) {
  ...
  await replaceGameTextFilesAtomically(project, record.candidate.textFiles);
