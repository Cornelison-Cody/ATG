export function formatAzureCodexJobStartError(status, body) {
  ...
  if (parsed?.code === "AuthorizationFailed") {
    ...
    return [
      ...
      details ? ` ${details}.` : "",
      " Run the one-time Codex job RBAC bootstrap from the deployment workflow, which grants Container Apps Jobs Operator on the Codex Job, then retry the build or plan request."
    ].join("");
  }
