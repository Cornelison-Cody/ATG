...
export function createAssetOptimizationPlan(assets = [], options = {}) {
  ...
  const normalized = [...new Map(assets.map(normalizeAsset).map((asset) => [asset.path, asset])).values()].sort((left, right) => left.path.localeCompare(right.path));
...
export async function runAssetOptimization(plan, optimize = async (output) => output) {
  ...
  for (const output of plan.outputs) {
    const result = await optimize(output);
    if (result && typeof result === "object" && result.path && result.sourcePath !== output.sourcePath) throw new Error("Derived output source does not match the optimization plan.");
...
export function createPreloadManifest(outputs = []) {
  const seen = new Set();
  const entries = outputs.filter((output) => output?.path && !seen.has(output.path) && seen.add(output.path)).map((output) => ({ path: output.path, sourcePath: output.sourcePath, kind: output.kind, bytes: output.bytes }));
