...
export function createMediaJob({ kind, prompt, projectId, provider, model, referenceConsent = false, billingMode = "managed" } = {}) {
  ...
  return {
    id: randomUUID(), kind, prompt: prompt.trim(), projectId, provider, model: model || defaultModel(kind, provider),
...
export async function runMediaJob(job, { generate, moderate = async () => ({ allowed: true }), store, billing = {}, onProgress = () => undefined, signal } = {}) {
  ...
  const promptModeration = await moderate({ job, phase: "prompt" });
  if (!promptModeration?.allowed) {
    job.status = "failed";
    job.result = { code: "moderation", message: promptModeration?.reason || "Media prompt was not approved." };
    update(job.result.message, "failed");
    return job;
  }
  ...
  try {
    ...
    const moderation = await moderate({ job, generated, phase: "output" });
...
function defaultModel(kind, provider) {
  return provider === "openai-image" || ["image", "character", "object", "sprite-variation", "animation-sheet"].includes(kind)
    ? "gpt-image-2"
    : "gpt-audio-1.5";
}
