...
export async function acceptMediaJob(project, id, metadata = null) {
  ...
  const asset = await uploadGameAsset(project, { content: Buffer.from(preview.bytes, "base64"), contentType: preview.contentType, filename: preview.filename });
...
export async function discardMediaJob(projectId, id) { return updateMediaJobForProject(projectId, id, (job) => { if (job.status === "accepted") throw new MediaJobStoreError("Accepted media cannot be discarded.", 409); job.status = "discarded"; job.result = { code: "discarded", message: "Media preview discarded." }; }); }
...
async function storeGeneratedAsset(_project, generated, provenance) { return { preview: { bytes: Buffer.from(generated.bytes).toString("base64"), contentType: generated.contentType, filename: generated.filename, provenance } }; }
