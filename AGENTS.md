# Repository Instructions

## GitHub Issue Defaults

When asked to create an issue or ticket for this repository, create it in `Cornelison-Cody/ATG` unless the user specifies another repository.

After creating the issue, add it to the GitHub project `TV Platform` owned by `Cornelison-Cody`, and set its `Status` field to `Backlog` by default. Only use a different project or status if the user explicitly asks for one.

## GitHub Issue Workflows

When asked to find or pick a ticket/issue to work on, ask clarifying questions as needed before implementing. After completing the work, submit a pull request ready for review, not in draft mode, and move the issue's `TV Platform` project status to `In Review`.
